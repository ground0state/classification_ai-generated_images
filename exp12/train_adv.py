...
# 保存用ディレクトリ作成
checkpoint_dir = join(c.work_dirs, c.exp)
os.makedirs(checkpoint_dir, exist_ok=True)
log_dir = join(c.work_dirs, c.exp, "tf_logs")
...
for epoch in range(1, c.num_epochs+1):
    ...
    total_data = 0
    for step, (inputs, labels, domain_labels) in enumerate(train_loader, start=1):
        inputs, labels = inputs.to(device), labels.to(device)
        outputs, domain_outputs = model(inputs)
        loss = criterion(outputs[domain_labels==0], labels[domain_labels==0])
        domain_loss = domain_criterion(domain_outputs[domain_labels==1], labels[domain_labels==1])
